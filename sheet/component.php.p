...
$LOGIN = $pieces[0];
$PWS = $pieces[1];
AddMessage2Log("Login:" . $LOGIN . " Password:" . $PWS . "...", "FirstUpload");
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
if ($xml_upload->xpath('//Классификатор/Группы/Группа/Группы/Группа'))
{
    ...
    foreach ($xml_upload->xpath('//Классификатор/Группы/Группа/Группы/Группа') as $proto_items)
    {
        ...
        //if (md5_file($WORK_DIR_NAME . $path_telpic) == md5_file($WORK_DIR_UPLOAD . 'telpic' . '/' . $name_telpic)) continue;
        ...
        if ($THERE_IS_FIRST_UPLOAD == FALSE)
        {
            ...
            while ($arSection = $rsSections->Fetch())
            {
                ...
                if (trim($str_one) == trim($str_two))
                {
                    ...
                    AddMessage2Log("В разделе с ID = " . $arSection['ID'] . " добавление  картиники с ID = " . $fid . "! " . var_dump($result_file) . "! " . var_dump($dir_path_telpic) . "! " . var_dump($$arSection), "PictureUpload");
                }
            }
        }
        /**
...
if ($xml_upload->xpath('//ПакетПредложений/Предложения/Предложение'))
{
    ...
    foreach ($xml_upload->xpath('//ПакетПредложений/Предложения/Предложение') as $offer_items)
    {
        ...
        //if (md5_file($WORK_DIR_NAME . $path_acupic_1) == md5_file($WORK_DIR_UPLOAD . 'acupic' . '/' . $temp_offer_items_article[0] . '/' . $name_acupic_1))
        //    if (md5_file($WORK_DIR_NAME . $path_acupic_2) == md5_file($WORK_DIR_UPLOAD . 'acupic' . '/' . $temp_offer_items_article[0] . '/' . $name_acupic_2))
        //        if (md5_file($WORK_DIR_NAME . $path_acupic_3) == md5_file($WORK_DIR_UPLOAD . 'acupic' . '/' . $temp_offer_items_article[0] . '/' . $name_acupic_3))
        //            if (md5_file($WORK_DIR_NAME . $path_acupic_4) == md5_file($WORK_DIR_UPLOAD . 'acupic' . '/' . $temp_offer_items_article[0] . '/' . $name_acupic_4)) continue;
...
if (($THERE_IS_NO_MATCH_OFFERS) || ($xml_offers->xpath('//dataWs')) || ($xml_prices->xpath('//dataWs')) || ($xml_instock->xpath('//dataWs')))
{
    ...
    shell_exec("echo yes>sleep;sleep 60");
